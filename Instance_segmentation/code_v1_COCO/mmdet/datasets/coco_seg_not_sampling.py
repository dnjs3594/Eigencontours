...
class Coco_Seg_Dataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        flip_dir = "/data/PolarMask-master_SCD_v3/tools/"
        if flip:
            load_rs = load_pickle(flip_dir + "data_pickle/{}".format(img_info['file_name'].rstrip('.jpg')))
            ...
            load_rs = load_pickle("data_pickle/{}".format(img_info['file_name'].rstrip('.jpg')))
        ...
        _labels, _bbox_targets, _mask_targets, _c_targets, gt_cs, gt_rs, _c_targets_comp, gt_pos = self.polar_target_single(
            gt_bboxes,gt_masks,gt_labels,concat_points, concat_regress_ranges, load_rs, load_cs, load_pos)
        ...
        #
        # save_pickle(dir_name="data_pickle/", file_name=img_info['file_name'].rstrip('.jpg'), data=gt_rs)
    ...
    def polar_target_single(self, gt_bboxes, gt_masks, gt_labels, points, regress_ranges, load_rs, load_cs, load_pos):
        ...
        for p,id in zip(pos_inds, pos_mask_ids):
            ...
            # cs, ray = self.get_360_coordinates(x, y, pos_mask_contour)
        ...
        return labels, bbox_targets, mask_targets, c_targets, gt_cs, r_targets, comp_c_target, torch.stack((pos_inds, pos_mask_ids))
    ...
    def get_72_coordinates(self, c_x, c_y, pos_mask_contour):
        ct = pos_mask_contour[:, 0, :]
        x = ct[:, 0] - c_x
        y = ct[:, 1] - c_y
        # angle = np.arctan2(x, y)*180/np.pi
        ...
        for a in range(0, 360, 5):
            if not a in new_coordinate.keys():
                ...
                distances[a//5] = new_coordinate[a]
