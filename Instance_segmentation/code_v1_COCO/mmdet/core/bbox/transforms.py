...
def bbox_mask2result(bboxes, masks, labels, num_classes, img_meta, rescale=True):
    """Convert detection results to a list of numpy arrays.
    ...
    for i in range(masks.shape[0]):
        ...
        contours[label].append(masks[i].cpu().numpy())
    ...
    if bboxes.shape[0] == 0:
        ...
        contour_results = [
            np.zeros((0, 2, 360), dtype=np.float32) for i in range(num_classes - 1)
        ]
        return bbox_results, mask_results, contour_results
    else:
        bboxes = bboxes.cpu().numpy()
        labels = labels.cpu().numpy()
        bbox_results = [bboxes[labels == i, :] for i in range(num_classes - 1)]
        contour_results = [np.array(contours[i]) for i in range(num_classes - 1)]
        return bbox_results, mask_results, contour_results
