...
class Eigencontours_Head(nn.Module):
    ...
    def __init__(self,
        ...
        super(Eigencontours_Head, self).__init__()
        ...
        self.angles = torch.range(0, 359, 1).cuda() / 180 * math.pi
        ...
        self.dist = math.sqrt(1280 * 1280 / 4 + 768 * 768 / 4) / 10
        self.loss_c_weight = torch.arange(1, 37, 1).float() / 36
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            if not self.use_dcn:
                ...
                if self.norm_cfg:
                    self.c_convs.append(build_norm_layer(self.norm_cfg, self.feat_channels)[1])
                self.c_convs.append(nn.ReLU(inplace=True))
        ...
        self.polar_c = nn.Conv2d(self.feat_channels, 36, 3, padding=1)
        ...
        self.scales_c = nn.ModuleList([Scale(1.0) for _ in self.strides])
    ...
    def init_weights(self):
        ...
        normal_init(self.polar_c, std=0.01)

    def forward(self, feats):
        return multi_apply(self.forward_single, feats, self.scales_bbox, self.scales_mask, self.scales_c)

    def forward_single(self, x, scale_bbox, scale_mask, scale_c):
        cls_feat = x
        reg_feat = x
        mask_feat = x
        c_feat = x
        ...
        for c_layer in self.c_convs:
            c_feat = c_layer(c_feat)
        c_pred = scale_c(self.polar_c(c_feat))


        return cls_score, bbox_pred, centerness, mask_pred, c_pred

    @force_fp32(apply_to=('cls_scores', 'bbox_preds', 'mask_preds', 'c_pred', 'centernesses'))
    def loss(self,
             cls_scores,
             bbox_preds,
             centernesses,
        ...
        labels, bbox_targets, mask_targets, c_targets = self.polar_target(all_level_points, extra_data)
        ...
        flatten_c_preds = [
            c_pred.permute(0, 2, 3, 1).reshape(-1, 36)
        ...
        flatten_c_preds = torch.cat(flatten_c_preds)  # [num_pixel]
        ...
        flatten_c_targets = torch.cat(c_targets)  # [num_pixel, 36]
        ...
        pos_c_preds = flatten_c_preds[pos_inds]
        pos_mask_preds = torch.clamp((torch.matmul(self.U[:, :36].cuda(), pos_c_preds.T) * self.dist).T, min=1e-6)
        # pos_mask_preds_dict = {}
        # pos_mask_targets_dict = {}
        if num_pos > 0:
            pos_bbox_targets = flatten_bbox_targets[pos_inds]
            # pos_mask_targets = flatten_mask_targets[pos_inds]
            pos_c_targets = flatten_c_targets[pos_inds]
            pos_mask_targets = torch.clamp((torch.matmul(self.U[:, :36].cuda(), pos_c_targets.T) * self.dist).T, min=1e-6)
            pos_centerness_targets = self.polar_centerness_target(pos_mask_targets)
            # loss_c = 0
            # for i in range(36):
            #     pos_mask_targets_dict[i + 1] = torch.clamp((torch.matmul(self.U[:, 0:i+1].cuda(), pos_c_targets[:, 0:i+1].T) * self.dist).T, min=1e-6)
            #     # pos_mask_preds_dict[i + 1] = torch.clamp((torch.matmul(self.U[:, 0:i+1].cuda(), torch.cat((pos_c_targets[:, 0:i], pos_c_preds[:, i:i+1]), dim=1).T) * self.dist).T, min=1e-6)
            #     pos_mask_preds_dict[i + 1] = torch.clamp((torch.matmul(self.U[:, 0:i + 1].cuda(), pos_c_preds[:, 0:i+ 1].T) * self.dist).T, min=1e-6)
            #     loss_c += self.loss_mask(pos_mask_preds_dict[i+1],
            #                             pos_mask_targets_dict[i+1],
            #                             weight=pos_centerness_targets,
            #                             avg_factor=pos_centerness_targets.sum())

            loss_c = self.loss_mask(pos_mask_preds,
                                     pos_mask_targets,
                                     weight=pos_centerness_targets,
                                     avg_factor=pos_centerness_targets.sum())
            ...
            # loss_c = self.loss_c(pos_c_preds,
            #                      pos_c_targets,
            #                      weight=pos_centerness_targets,
            #                      avg_factor=pos_centerness_targets.sum())
            ...
            #                            pos_mask_targets,
            #                            weight=1, avg_factor=1) * 0

            # loss_c = self.loss_c(pos_c_preds,
             #                     pos_c_targets)

            # loss_centerness =self.loss_centerness(pos_centerness,
            #                                         pos_centerness_targets) * 0
        else:
            loss_bbox = pos_bbox_preds.sum()
            # loss_mask = pos_mask_preds.sum()
            loss_centerness = pos_centerness.sum()
            loss_c = pos_c_preds.sum()
    ...
    def polar_target(self, points, extra_data):
        ...
        labels_list, bbox_targets_list, mask_targets_list, c_targets_list = extra_data.values()
        ...
        c_targets_list = [
            c_targets.split(num_points, 0)
            for c_targets in c_targets_list
        ]
        ...
        for i in range(num_levels):
            ...
            concat_lvl_c_targets.append(
                torch.cat(
                    [c_targets[i] for c_targets in c_targets_list]))

        return concat_lvl_labels, concat_lvl_bbox_targets, concat_lvl_mask_targets, concat_lvl_c_targets
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            c_pred_list = [
                c_preds[i][img_id].detach() for i in range(num_levels)
            ]
    ...
    def get_bboxes_single(self,
        ...
        mlvl_centerness = []
        # mlvl_c = []
        for cls_score, bbox_pred, mask_pred, centerness, c_pred, points in zip(
                cls_scores, bbox_preds, mask_preds, centernesses, c_preds, mlvl_points):
            ...
            c_pred = c_pred.permute(1, 2, 0).reshape(-1, 36)
            ...
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                ...
                c_pred = c_pred[topk_inds, :]
                scores = scores[topk_inds, :]
                centerness = centerness[topk_inds]
            bboxes = distance2bbox(points, bbox_pred, max_shape=img_shape)
            # masks = distance2mask(points, mask_pred, self.angles, max_shape=img_shape)
            self.dist = math.sqrt(img_shape[0] * img_shape[0] / 4 + img_shape[1] * img_shape[1] / 4) / 10
            try:
                masks = c2mask(points, c_pred, self.angles, max_shape=img_shape, U=self.U, dist=self.dist)
            except:
                masks = torch.FloatTensor([]).cuda().view(-1, 2, 360)
        ...
        if rescale:
            ...
            try:
                scale_factor = torch.Tensor(scale_factor)[:2].cuda().unsqueeze(1).repeat(1, 360)
                _mlvl_masks = mlvl_masks / scale_factor
            except:
                _mlvl_masks = mlvl_masks / mlvl_masks.new_tensor(scale_factor)
        else:
            _mlvl_bboxes = mlvl_bboxes
            _mlvl_masks = mlvl_masks
...
def c2mask(points, distances, angles, max_shape=None, U=None, dist=1):
    '''Decode distance prediction to 36 mask points
    Args:
        points (Tensor): Shape (n, 2), [x, y].
        distance (Tensor): Distance from the given point to 36,from angle 0 to 350.
    ...
    '''
    num_points = points.shape[0]
    points = points[:, :, None].repeat(1, 1, 360)
    c_x, c_y = points[:, 0], points[:, 1]
    ...
    distances = (torch.matmul(U[:, :36].cuda(), distances.T) * dist).T
