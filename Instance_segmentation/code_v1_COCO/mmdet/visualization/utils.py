...
def get_batch_statistics_contour(outputs, targets, iou_threshold, scale):
    """Compute true positives, predicted scores and predicted labels per batch."""
    batch_metrics = []
    polygon_pred = []
    polygon_gt = []
    ...
    for i, output in enumerate(outputs):
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label, pred_coeff, pred_center) in enumerate(zip(pred_boxes, pred_labels, pred_coeffs, pred_centers)):
                ...
                # iou, box_index = bbox_iou(pred_box.unsqueeze(0), target_boxes).max(0)
                iou_= coef_polygon_contour(pred_coeff, pred_center, pred_box, target_polygons_x, target_polygons_y, scale)
                iou, box_index = iou_.max(1), iou_.argmax(1)
                if iou >= iou_threshold and box_index not in detected_boxes:
                    true_positives[pred_i] = 1
                    detected_boxes += [box_index]
...
def coef_polygon_contour(pred_coef_l, pred_center_l, pred_bbox_l, gt_points_xs_l, gt_points_ys_l, scale):
    """Calculate Intersection-Over-Union(IOU) of pred coefs(Reconstructed) and gt polygon points
    Parameters
    ----------
    pred_coef_l : numpy.ndarray
    ...
    polygon_bs = np.concatenate((gt_points_xs_l, gt_points_ys_l), axis=-1)  # M, 360 ,2
    polygon_as = coef_trans_polygon_ESR(pred_coef_l, pred_bbox_l, pred_center_l, scale)
    polygon_bs, polygon_as = make_polygon(polygon_bs, polygon_as)

    iou_list = []
    for i in range(polygon_bs.shape[0]):
        try:
            iou_list.append(batched_f_measure(polygon_bs[i:i+1], polygon_as))
        except:
            a=1
    return np.array(iou_list).reshape(1, -1)
...
def coef_trans_polygon_ESR(coefs, bboxs, centers, scale):
    ...
    """
    top_k = 18
    U = to_np(load_pickle('/home/park/project/preprocessing/E02/output_SBD_train_v3_node_360_top_18/pickle/U'))[:, :top_k]
